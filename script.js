...
document.querySelector('#result').addEventListener('click', () => {
    ...
    dataSet().forEach((point) => {
        distances.push(Math.sqrt(((x - point.humudity) ** 2) + ((y - point.pressure) ** 2)))
    })
    ...
    if (redNeighbors > blueNeighbors) {
        document.querySelector('#output').innerHTML = 'Yağmur Görünmüyor'
        chart.data.datasets[0].pointBackgroundColor[chart.data.datasets[0].data.length - 1] = 'red'
    }else {
        document.querySelector('#output').innerHTML = 'Yağmur var gibi görünüyor'
